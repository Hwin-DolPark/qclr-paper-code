...
# root path
root_path = '/home/hpcmate/workspace/drpath4y/MQR/Medformer/data/PTB/ptb-diagnostic-ecg-database-1.0.0'
...
def R_Peaks(ecg_data):
    ...
    for ch in range(ecg_data.shape[1]):
        cleaned_ecg = nk.ecg_clean(ecg_data[:, ch], sampling_rate=250,
                                   method='neurokit')
        signals, info = nk.ecg_peaks(cleaned_ecg, sampling_rate=250,
    ...
    for tri in os.listdir(sub_path):
        if '.dat' in tri:
            ...
            try:
                ...
                print(
                    'The trial is invalid with trial path {}'.format(tri_path))
        ...
        if '.dat' in tri:
            ...
            try:
                ...
                if med.median() <= max_duration:
                    ...
                    print('The trial is an outlier with trial path {}'.format(
                        tri_path))
                    li_abnormal_trial.append(tri_path)
            except IndexError:
                print(
...
# main
feature_path = '/home/hpcmate/workspace/drpath4y/MQR/Medformer/data/PTB/Feature'
...
for sub_path in li_slc_sub:
    ...
    for tri in os.listdir(sub_path):
        if 'dat' in tri:
            ...
            if tri_path not in li_abnormal_trial:
                label = \
                wfdb.rdsamp(record_name=tri_path[:-4])[1]['comments'][4].split(
                    ':')[-1].strip()  # label
                ...
                trial = []
                for ch in range(ecg_data.shape[1]):
                    data = resampling(ecg_data[:, ch], freq=1000,
                                      kind='linear')
                    trial.append(data)
                trial = np.array(trial).T
                trial_norm = normalize(trial)
                samples = sample(trial_norm, max_duration=300)
                segmentations = sample2seg(samples,
                                           seg_size=1)  # several samples concat into a segmentation, seg_size=1 if one sample only
    ...
    if li_sub_segs != list():  # Not None list
        ...
        print(
            'The subject is None after preprocessing with the path {}'.format(
                tri_path))
...
# label.npy
label_path = '/home/hpcmate/workspace/drpath4y/MQR/Medformer/data/PTB/Label'
