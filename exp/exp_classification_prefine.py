...
class Exp_Classification_PreFine(Exp_Basic):
    ...
    def qclr_loss_dynamic(self, latent_vector, true_y, current_epoch,
                          ...
                          # Starting lower quantile for negatives
                          target_lower_threshold=0.4,
                          # Target lower quantile after annealing
        ...
        if lower_threshold >= upper_threshold or lower_threshold >= positive_quantile:
            ...
            # Decide how to handle: maybe return 0 loss, or adjust thresholds?
            # For now, let's just warn. Consider adjusting logic if this occurs often.
        ...
        # Temporarily replace irrelevant similarities (self, different label) with a value
        # that won't affect quantile calculation (e.g., -1 or NaN if handled)
        ...
        for i in range(batch_size):
            ...
            if anchor_relevant_sims.numel() > 0:
                # Use interpolation='linear' (default) or 'nearest' etc. if needed
        ...
        # Need to be careful: add sum_exp_neg only where exp_positives is non-zero
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, label, padding_mask) in enumerate(train_loader):
                ...
                if self.args.task_name == 'classification_qclr':
                    ...
                    loss = self.qclr_loss_dynamic(output_qclr, label.long(), epoch)
                else:
                    outputs = self.model(batch_x, padding_mask, None, None)
